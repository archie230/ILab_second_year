...
namespace cxx_containers {
            ...
            for (i = 0; i < rows_; i++) {
                ...
                for (j = 0; j < rows_; j++) {
                    if (j < i)
                        ...
                        if(arithmetic_traits::eq(L[i][i], 0)) {
                            L.set_zero();
                            U.set_zero();
                            return {L, U};
                        }
    ...
    /**
     * Solving A*X=b
     * @tparam T must be able to float point arithmetic
     * @param A
     * @param b
     * @return X
     */
    ...
    Matrix<T> SolveSystem(const Matrix<T>& A, const Matrix<T>& b) {
        if (b.cols() != 1 || A.rows() != A.cols() || b.rows() != A.rows())
    ...
    template <typename FwdIt,
            typename = std::_RequireInputIter<FwdIt>>
    Matrix(FwdIt, FwdIt, const std::pair<typename Matrix<value_type<FwdIt>>::size_type,
                                         typename Matrix<value_type<FwdIt>>::size_type>&) -> Matrix<value_type<FwdIt>>;
